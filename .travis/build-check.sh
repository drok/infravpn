...
if [ -z ${CHOST+x} ]; then
	if [ "${TRAVIS_OS_NAME}" = "linux" ]; then
		export EXTRA_CONFIG="${EXTRA_CONFIG:-} --enable-werror --enable-unit-tests=all"
	fi
	...
	./configure --with-crypto-library="${SSLLIB}" --host=${CHOST} --build=x86_64-pc-linux-gnu --enable-pkcs11 --disable-plugins || 
